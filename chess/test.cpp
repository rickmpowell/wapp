...
/**
 *  @file       test.cpp
 *  @brief      Test window and commands
 * 
 *  @details    The test panel on the desktop, along with some of the testing 
 *              primitives.
 * 
...
SZ WNLOG::SzIntrinsic(const RC& rcWithin)
{
...
/**
 *  @class CMDLEVELLOG
 *  @brief Cycle through the log levels.
 */

class CMDLEVELLOG : public CMD<CMDLEVELLOG, WAPP>
{
public:
    CMDLEVELLOG(WNLOG& wnlog, CYCLE& cycle) : 
        CMD(Wapp(wnlog.iwapp)), 
        wnlog(wnlog),
        cycle(cycle)
    {
    }
...
/**
 *  The toolbar on the test/log window
 */
...
TOOLBARLOG::TOOLBARLOG(WNLOG& wnlog) :
    ...
    cycleLog(*this, new CMDLEVELLOG(wnlog, cycleLog), wnlog.levelLog, 1, 20)
{
    btnSave.SetPadding(4);
    btnSave.SetLeit({ .lerole = LEROLE::ToolbarCmdButton, 
                      .lestretch = LESTRETCH::KeepAspect,
                      .leinterior = LEINTERIOR::ScaleInteriorToFit });
    btnCopy.SetPadding(0, -1, 0, -1);
    btnCopy.SetLeit({ .lerole = LEROLE::ToolbarCmdButton,
                      .lestretch = LESTRETCH::KeepAspect,
                      .leinterior = LEINTERIOR::ScaleInteriorToFit });
    btnClear.SetPadding(0, -2, 0, 2);
    btnClear.SetLeit({ .lerole = LEROLE::ToolbarCmdButton,
                       .lestretch = LESTRETCH::KeepAspect,
                       .leinterior = LEINTERIOR::ScaleInteriorToFit });
    
    cycleLog.SetFontHeight(14);
    cycleLog.SetLeit({ .lerole = LEROLE::ToolbarRight,
                       .lealignh = LEALIGNH::Right });
}
...
/**
 *  @fn void WAPP::RunPerftSuite(void)
...
/**
 *  @fn bool WAPP::RunOnePerftTest(const char tag[], const char fen[], const int64_t mpdcmv[],
 *  
        ...
        while (getline(is, epd)) {
            ...
            try {
                ...
                if (game.mpkeyvar.find("bm") != game.mpkeyvar.end()) {
                    ...
                    throw ERRAPP(rssErrEpdNoBestMove);
...
/**
 *  @fn void WAPP::RunAIProfile(void)
 *  @brief Run a single AI test on a known position for easy profiling
 */
...
/**
 *  @fn         WAPP::AnalyzePosition(void)
 *  @brief      Analyzes the current board position with the AI
 *
 *  @details    This is currently very rudimentary. It just installs the AI
 *              player into the game and calculates the best move. In the 
 *              future, this could be signiificantly more sophisticated.
 */

void WAPP::AnalyzePosition(void)
{
    /* TODO: shared with AI Test */
...
/**
 *  @class CMDPERFT
 *  @brief Command for the selection box in the perft dialog
 * 
 *  REVIEW: can I get rid of this?
 */
...
/**
 *  @fn DLGPERFT::DLGPERFT(WN& wnParent, WNLOG& wnlog)
 *  @brief Dialog constructor to run perft tests
 */
...
DLGPERFT::DLGPERFT(WN& wnParent, WNLOG& wnlog) :
    ...
    cycleDepth(*this, nullptr, wnlog.dPerft, 1, 20),
    btnok(*this)
{
    Init(wnlog);
...
SZ DLGPERFT::SzIntrinsic(const RC& rcWithin)
{
    return SZ(800, 400);
}
...
SZ VSELPERFT::SzIntrinsic(const RC& rcWithin)
{
